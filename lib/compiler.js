...
var Compiler = function (cfg) {
    ...
    var _doc = function () {
        ...
        var dir = cfg.tahoe.docDir;
        if (!fs.existsSync(dir)) {
            fs.mkdirSync(dir);
        }
        var docTemplate = require("../doc/dest/doc-schema");
        docTemplate.render("doc:index", {schema:ctx.schema}, null, function(err, html) {
            fs.writeFileSync(path.join(dir, "index.html"), html);
            fs.createReadStream(path.join(__dirname, '../doc/dest/public/doc.css')).pipe(fs.createWriteStream(path.join(dir, "doc.css")));
        });

        for (var block in ctx.schema.blocks) {
            docTemplate.render("doc:block", {block:ctx.schema.blocks[block]}, null, function(err, html) {
                var n = block.replace(/\//g, "_").replace(":", "_")+".html";
                fs.writeFileSync(path.join(dir, n), html);
            });
        }
    };
